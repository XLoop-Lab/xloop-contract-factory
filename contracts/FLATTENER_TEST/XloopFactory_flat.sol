...
// File: contracts/INTF/IXLoopToken.sol
...
contract XloopFactory is CloneFactory, XLTContext, Ownable {
    ...
    function createPair(
        ...
        if (AllLK[P_TOKEN_A] != address(0)) {
            ...
            if (CheckWhiteList == true && _msgSender() != owner()) {
                require(WhiteList[P_TOKEN_A_ROOT] == true, "TOKEN_INVALID");
            }
        }
        require(
        ...
        if (P_TOKEN_A_ROOT == Xloop || XloopGATEList[P_TOKEN_A_ROOT] == true) {
            _is_reward = true;
        }
        IXLoopLK(_LK).initialize(_PAIR, _name, _symbol, _decimal, _is_reward);
        IXLoopPair(_PAIR).initialize(
            P_TOKEN_A,
            P_TOKEN_A_ROOT,
            _LK,
            _diffdec,
    ...
    /* onlyOwner */
    function setWhiteList(address P_TOKEN_A, bool P_IS_ON)
        external
        onlyOwner
        lock
    {
